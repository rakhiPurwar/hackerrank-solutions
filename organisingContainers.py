...
def organizingContainers(container):
    ...
    a = []     #declaring 2 empty lists for row sum
    b = []     #for colummn sum
    n= len(container)   #container is a 2d matrix so to get its length.
    for i in range(n): 
        rsum=0    #decalring the variables for individual row and column sum     
        csum=0    #for column sum
        for j in range(n):
            rsum+=container[i][j]     #row element
            csum+=container[j][i]     #column element
        a+=[rsum]
        b+=[csum]
